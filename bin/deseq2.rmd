...
## DESeq2 table FEMALE vs MALE
```{r, echo=F}
sex_res <- results(dds, contrast=c("sex", "F", "M"), alpha = 0.01)
sex_res$gene <- rownames(sex_res)
sex.df <- as.data.frame(sex_res)
sex.df <- sex.df %>% dplyr::mutate(
  regulation=ifelse(log2FoldChange < 0, "Downregulated", "Upregulated")
)
round_cols <- names(sex.df)[sapply(sex.df, is.numeric)]
datatable(sex.df,
  extensions = "Buttons",
  options = list(
    dom = "Bfrtip",
...
```
## DESeq2 table FEMALE vs MALE remove padj NA
```{r, echo=F}
sex.rm.na <- sex.df[!is.na(sex.df$padj),]
round_cols <- names(sex.rm.na)[sapply(sex.rm.na, is.numeric)]
datatable(sex.rm.na,
  extensions = "Buttons",
  options = list(
    dom = "Bfrtip",
