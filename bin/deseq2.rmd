...
params:
  ...
  salmon_path: "placeholder"
  deg_path: "placeholder"
title: |
...
Usage:
rmarkdown::render("deseq2.rmd", params = list(study = "Study Name"))
-->
...
# params <- list(
#     "G:/My Drive/EPI-321/off_target/pipelines/epic-rnaseq/epic-rnaseq/JIRA_BDS-1008_Mouse_3m/mouse_3m_samplesheet_deseq2.csv",
#     "G:/My Drive/EPI-321/off_target/pipelines/epic-rnaseq/epic-rnaseq/JIRA_BDS-1008_Mouse_3m/salmon",
#     "G:/My Drive/EPI-321/off_target/pipelines/epic-rnaseq/epic-rnaseq/JIRA_BDS-1008_Mouse_3m/deseq2/JIRA_BDS-1008_Mouse_3m_degs.tsv"
# )

# names(params) <- c("metadata_path", "salmon_path", "deg_path")
...
pcaBatch <- plotPCA(vsd, intgroup=c("sample", "condition"), returnData = T)
ggplot(pcaBatch, aes(x = PC1, y = PC2, color = condition)) + 
  geom_point(size=3) +
  ggtitle("PCA with batch effects") +
  scale_color_manual(values = c("CONTROL" = "#619CFF", "EPI321" = "#F8766D"))

# batch corrected pca
pcaData <- plotPCA(vsd, intgroup=c("sample", "condition"), returnData = T)
pcaData <- limma::removeBatchEffect(pcaData)
ggplot(as.data.frame(pcaData), aes(x = PC1, y = PC2, color = condition)) + 
  geom_point(size =3) +
