...
metadata <- metadata[c("sample", "condition", "dose")]
datatable(metadata)
...
ggplot(pca_obj, aes(x = PC1, y = PC2, color = condition)) + 
  geom_point(size=3) + 
  coord_fixed(ratio=1,
    xlim=c(min(pca_obj$PC1, pca_obj$PC2), max(pca_obj$PC1, pca_obj$PC2)),
    ylim=c(min(pca_obj$PC1, pca_obj$PC2), max(pca_obj$PC1, pca_obj$PC2))
  ) +
  geom_text(aes(label=rownames(pca_obj)), hjust=0, vjust=0, show.legend=FALSE) + 
  theme(
...
l2fc_thresh <- params$l2fc
padj_thresh <- params$padj
...
if ("EPI321" %in% metadata$condition & "CONTROL" %in% metadata$condition) {
  ...
  res <- results(dds, contrast=c("condition","EPI321","UNTREATED"), alpha = 0.01) 
} else if ("EPI321" %in% metadata$condition & "STOP" %in% metadata$condition) {
  res <- results(dds, contrast=c("condition","EPI321","STOP"), alpha = 0.01) 
} else if ("STOP" %in% metadata$condition & "UNTREATED" %in% metadata$condition) {
  res <- results(dds, contrast=c("condition","STOP","UNTREATED"), alpha = 0.01) 
} else if ("CONTROL" %in% metadata$condition & "TREATMENT" %in% metadata$condition) {
  res <- results(dds, contrast=c("condition","TREATMENT","CONTROL"), alpha = 0.01) 
} else {
...
total_sig_degs <- nrow(subset(res.df, padj < padj_thresh & abs(log2FoldChange) > l2fc_thresh))
```

**`r total_sig_degs`** DEGs were identified with adjusted p-value < `r padj_thresh` and absolute log2FoldChange > `r l2fc_thresh`.

## Volcano plot
log2FoldChange defined by log2(`r numerator`/`r denominator`).  
p-value refers to the adjusted p-value.
...
EnhancedVolcano(res,
    ...
    x = "log2FoldChange",
    y = "padj",
    caption = paste(" Total genes:", nrow(res.df),
      " log2FC > ", l2fc_thresh, ": ", nrow(subset(res.df,log2FoldChange > l2fc_thresh)),
      " log2FC < -", l2fc_thresh, ": ", nrow(subset(res.df,log2FoldChange < -l2fc_thresh)),
      " significant log2FC > ", l2fc_thresh, ": ", nrow(subset(res.df, log2FoldChange > l2fc_thresh & padj < padj_thresh)),
      " significant log2FC < -", l2fc_thresh, ": ", nrow(subset(res.df, log2FoldChange < -l2fc_thresh & padj < padj_thresh))
      ),
    captionLabSize = 11,
...
## DESeq2 table EPI321 vs CONTROL
...
## DESeq2 table LOW vs HIGH
```{r, echo=F}
dose_res <- results(dds, contrast=c("dose", "LOW", "HIGH"), alpha = 0.01)
dose_res$gene <- rownames(dose_res)
dose.df <- as.data.frame(dose_res)
dose.df <- dose.df %>% dplyr::mutate(
  regulation=ifelse(log2FoldChange < 0, "Downregulated", "Upregulated")
)
round_cols <- names(dose.df)[sapply(dose.df, is.numeric)]
datatable(dose.df,
  extensions = "Buttons",
  options = list(
    dom = "Bfrtip",
...
```
## DESeq2 table LOW vs HIGH remove padj NA
```{r, echo=F}
dose.rm.na <- dose.df[!is.na(dose.df$padj),]
round_cols <- names(dose.rm.na)[sapply(dose.rm.na, is.numeric)]
datatable(dose.rm.na,
  extensions = "Buttons",
  options = list(
    dom = "Bfrtip",
    buttons = c("copy", "csv", "excel", "pdf", "print")
